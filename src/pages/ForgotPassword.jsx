      ...
      <section className="relative z-10 mx-auto grid min-h-[calc(100vh-4rem)] w-full max-w-6xl items-center gap-8 lg:grid-cols-[minmax(0,0.9fr)_minmax(380px,0.72fr)]">
        <div className="glass-panel relative overflow-hidden rounded-[28px] border border-cyan-300/20 bg-surface p-7 shadow-[0_30px_90px_rgba(2,6,23,0.38)] backdrop-blur-2xl">
          <div className="mb-7 flex items-center gap-3">
            ...
            <div>
              <p className="m-0 text-sm font-semibold text-accent">Secure Recovery Console</p>
              <span className="text-xs font-medium text-secondary">Identity reset monitored by AI signals</span>
            </div>
          </div>

          <div className="relative min-h-[390px] overflow-hidden rounded-3xl border border-line bg-control p-5">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_22%,rgba(14,165,233,0.24),transparent_34%),linear-gradient(180deg,var(--app-bg-secondary),var(--app-bg-secondary))]" />
            <div className="relative z-10 grid gap-5">
              <div className="flex items-center justify-between rounded-2xl border border-cyan-300/20 bg-cyan-400/10 p-4">
                <div className="flex items-center gap-3 text-accent"><KeyRound size={22} /><strong>{resetMode ? "Password Update" : "Reset Link Scan"}</strong></div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="rounded-3xl border border-line bg-control p-5">
                  <ScanFace className="mb-5 text-accent" size={34} />
                ...
                <div className="rounded-3xl border border-line bg-control p-5">
                  <div className="mb-5 flex items-center gap-2 text-sm font-semibold text-accent"><AudioWaveform size={18} /> Recovery Signal</div>
              ...
              <div className="rounded-3xl border border-line bg-control p-5">
                <div className="mb-4 flex items-center gap-2 text-sm font-semibold text-accent"><BrainCircuit size={18} /> Reset Protocol</div>
        ...
        <form className="ai-login-frame relative overflow-hidden rounded-[30px] border border-cyan-300/30 bg-surface p-6 shadow-[0_35px_100px_rgba(2,6,23,0.45),0_0_45px_rgba(6,182,212,0.16)] backdrop-blur-2xl sm:p-8" onSubmit={handleSubmit(resetMode ? onUpdatePassword : onSendLink)}>
          ...
          <div className="relative z-10">
            <div className="mb-8 grid justify-items-center text-center">
              ...
              <h1 className="m-0 text-3xl font-bold text-primary">{resetMode ? "Update Password" : "Forgot Password"}</h1>
              <p className="mt-3 text-sm font-medium text-accent/80">{resetMode ? "Create a new secure password for your monitored account." : "Enter your email to receive a secure reset link."}</p>
            </div>
            ...
            <div className="grid gap-5">
              {resetMode ? (
                <>
                  <label className="grid gap-2">
                    ...
                    <div className="flex min-h-[56px] items-center gap-3 rounded-2xl border border-cyan-200/20 bg-control px-4 text-primary shadow-[inset_0_0_24px_rgba(15,23,42,0.6)] transition focus-within:border-cyan-300/70">
                      ...
                      <input className="w-full border-0 bg-transparent outline-none placeholder:text-secondary" placeholder="New password" type={showPassword ? "text" : "password"} {...register("password", { required: "Password is required", minLength: { value: 6, message: "Password must be at least 6 characters" } })} />
                ...
                <label className="grid gap-2">
                  <span className="text-sm font-semibold text-primary">School ID / Email</span>
                  <div className="flex min-h-[56px] items-center gap-3 rounded-2xl border border-cyan-200/20 bg-control px-4 text-primary shadow-[inset_0_0_24px_rgba(15,23,42,0.6)] transition focus-within:border-cyan-300/70">
