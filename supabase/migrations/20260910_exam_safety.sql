...
create trigger touch_exams_updated_at before update on public.exams for each row execute function public.touch_updated_at();
...
declare eid uuid; old_exam public.exams; q jsonb; starts timestamptz; ends timestamptz; is_draft boolean; exam_title text; course_uuid uuid;
begin
 ...
 if p_id is not null then
  ...
  update public.exams set title=exam_title,exam_title=exam_title,course_id=course_uuid,description=p_exam->>'description',semester=p_exam->>'semester',exam_type=p_exam->>'exam_type',course=p_exam->>'course',exam_settings=p_exam->'exam_settings',duration=nullif(p_exam->>'duration','')::integer,time_limit=nullif(p_exam->>'duration','')::integer,questions_count=jsonb_array_length(p_questions),status=p_exam->>'status',approved_at=null,rejected_at=null,submitted_at=case when p_exam->>'status'='Pending Review' then now() else null end where id=p_id returning id into eid;
  ...
  values(course_uuid,exam_title,exam_title,p_exam->>'description',p_exam->>'semester',p_exam->>'exam_type',p_exam->>'course',p_exam->'exam_settings',nullif(p_exam->>'duration','')::integer,nullif(p_exam->>'duration','')::integer,jsonb_array_length(p_questions),p_exam->>'status',auth.uid(),auth.uid(),case when p_exam->>'status'='Pending Review' then now() else null end) returning id into eid;
 end if;
